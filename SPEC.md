...
- Speaker notes: `<aside class="notes">`. Two markers inside them are timing, never words: `⟨CLICK⟩` cuts the notes into beats, one per build step (BUILDS), and `⟨PAUSE⟩` holds a let-it-sink-in beat where it stands (PRESENTING). **Any spelling a person writes is the same marker** — `[click]`, `<click>`, `[pause]`, `<pause>`, in any case, typed as text, escaped (`&lt;pause&gt;`), or written as an element in the deck's HTML (which the runtime reads as the marker, not as the nothing `textContent` sees); so is a marker whose brackets are entities (`&#10216;CLICK&#10217;`). `[slow]…[/slow]` and `<slow>…</slow>` are read the same way and never spoken. Only those exact words match: `[1]`, `[ ]` and `<script>` in a note stay prose. The canonical forms read, and hash, exactly as they always have; a spelling that used to be spoken aloud is now the marker, so a recorded slide that carried one is stale once.
