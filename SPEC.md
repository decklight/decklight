...
- Keyboard: `→/←/Space` steps+slides, `Home/End`, `O` overview grid, `B` blackout, `F` fullscreen, `T` theme picker (`,`/`.` cycle the theme, `[`/`]` cycle the font), `L`/`⇧L` cycle the current slide's layout (author mode — a persisted deck edit), `E` toggle element edit mode — right-click a slide element or its background for a menu (author mode), `Z`/`⇧Z` undo/redo deck edits (author mode), `A` ask an AI agent to edit the deck (author mode), `/` command palette, **`⎵` narration play/pause** and **`V`** the panel that holds everything else about the voice — track, live voice, engine, tone, character, recording, captions, speed. Space is the verb because start/pause/resume is one thing a presenter does mid-talk, and it is **conditional**: on a deck with no voice it advances exactly as it always has, so nothing is taken from a deck that never narrates. `→`/`PageDown`/click always advance. `P` is the same verb, kept for the hands that learned it. The predicate is answered synchronously — a track configured, narration already running, or a bridge that has already replied — because a keypress cannot wait on a probe, and an un-probed bridge means Space keeps advancing rather than swallowing the key. `<`/`>` change the voice speed in 0.25× steps, 0.25–2×, persisted per deck — YouTube's shortcut, `C` captions, `W` pen / `⇧W` laser pointer (draw on the slide, `⌫` clears), `J` progress bar, `D` debug log, `` ` `` messages, `G` slide/module finder (deliberately not `⌘F` — browser find stays untouched), `H` deck history — the commits, which of them are only on this machine, a live preview, and `⏎` to restore one (author mode; `R` is the same door). In **present** mode `H` is the deck update instead (`PRESENT#UPSTREAM`), `?` help overlay.
