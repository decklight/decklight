...
- **Print/PDF**: `?print` renders all slides sequentially, every build complete, terminal casts fully expanded, one slide per page (`@media print` CSS). No JS needed after layout. Two handout variants restructure the same fully-built render onto portrait pages (one `.print-page` per sheet): `?print=handout` groups slides three to a page, each scaled slide beside a column of ruled note-taking lines (page count = ⌈N/3⌉); `?print=notes` gives one page per slide with that slide's speaker notes rendered underneath — slides without notes keep their page with an empty notes block.
...
decklight/
  ...
  cli/           decklight.mjs (dispatcher: rec/refresh/export/bundle/publish/tts/lipsync/edit/dev) + rec.mjs, bundle.mjs, publish.mjs, edit.mjs, dev.mjs, agents.mjs (AI-agent roster)
