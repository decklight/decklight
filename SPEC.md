...
- **Module markers** (optional): `data-module="<title>"` on a section marks the
  ...
  slide finder lists the chapters ahead of the slides (`PRESENTING`), and the
...
- **Slide finder**: reached from the palette (`/` → Enter, or the search fallback) — a find-a-slide overlay with the picker's anatomy — a query bar and result list on the left, a live preview of the selected slide on the right. Typing filters as an AND over the query's words against each slide's text; slides whose **title** contains every word rank first, body-only matches follow, and every match is listed as `<slide number> · <title>` (slides without a heading fall back to their leading text). `↑/↓`/hover browse (the preview swaps live), `Enter`/click jumps to the slide, `Backspace` edits, `Esc` clears the query then closes. The preview reuses the picker's lazy embedded-deck mechanism — the iframe boots once (carrying the active theme, generated/custom included via `?gen=`), then selections postMessage `{__decklightPreview: {goto: [slide, step]}}` into it; no reload per candidate. **A merged deck's chapters lead the list**: a deck
carrying in-file `data-module` markers (`DECK_ANATOMY`) gets one `▸ <title> —
module` row per chapter ahead of the slides, styled as a playlist's module rows
are and reading the same, because they mean the same thing to somebody looking
for one — what differs is that `⏎` here is a `goto()` in this file rather than a
page load, which the caption says (`module — <title> (slide N of this deck)`).
Chapters are ordinary title matches while typing, so a query still searches one
list. Only for the in-file shape: a deck with `config.playlist` is untouched,
the two never coexisting. Without this, a hundred-slide merge showed its five
chapter headings as five ordinary rows among ninety-five — findable if you knew
their titles, invisible if you did not.
