...
**Hidden slides** (`HIDDEN_SLIDES`): `<section data-hidden>` is a slide the audience does not see. It **keeps its number** — as in PowerPoint — so everything that numbers slides file-side (comments, review anchors, history) stays in step; presenting steps over it, a deep link onto it lands on the nearest shown neighbour in the direction of travel, the overview shows it dimmed and badged so an author can find it, and `pdf`, `video` and `voiceover` produce nothing for it. `?all` presents hidden slides too — never the default, because hidden is hidden from the audience and the default is the audience. `decklight import` keeps a PowerPoint hidden slide as one rather than dropping it.
