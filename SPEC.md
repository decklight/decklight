...
| shapes and connectors (a diagram somebody **drew**) | one themed SVG in the slide's own coordinates — boxes, ellipses, diamonds and triangles by their preset, arrows stopping at the boxes' edges, the shapes' words inside them rather than beside them as stray bullets. A **grouped** shape's own transform is in its group's child space (`a:chOff`/`a:chExt`), and only the group's frame says where on the slide that is — so frames are composed through every nesting level, or a group anyone moved or resized after grouping would come across where it sat before. The test is deliberately strict, because turning an ordinary slide into a strange picture is worse than not trying: **two placed, non-placeholder shapes and one connector that names the shapes at both of its ends** (`a:stCxn`/`a:endCxn` — what PowerPoint writes when somebody attaches an arrow to a box). Short of that nothing changes, except that a slide which LOOKED like a drawing (three placed shapes, or two with a line between them) now says its arrangement did not survive; two text boxes side by side stay silent, being a layout |
