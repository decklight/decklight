...
- Keyboard: `→/←/Space` steps+slides, `Home/End`, `O` overview grid, `B` blackout, `F` fullscreen, `T` theme picker (`,`/`.` cycle the theme, `[`/`]` cycle the font), `/` command palette, `V` narration (`N` picks track / live voice / tone), `C` captions, `D` debug log, `?` help overlay.
