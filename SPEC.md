...
- **Two shapes of one deck** (#517). The shape you *author* is the one above: `decklight.js`, `decklight.css` and a theme referenced as siblings the deck does not ship — `decklight init` writes exactly that (marked `data-decklight-runtime="js|css"`, with `data-decklight-version` recording the version it was written for), `author`, `present` and every render answer those references from the installed package (`PRESENT`), the runtime is always the installed one, and a git history of the deck is a history of slides. The shape you *hand over* is self-contained: `bundle` and `publish` embed the runtime, stylesheet and themes at that moment, so the file opens from disk, mails, and still plays in ten years — pinned to the version that embedded it, signed and audited as those bytes (`INTEGRITY`). `init --inline` and `import --inline` write the self-contained shape directly for anyone who needs a file that double-clicks; `upgrade --link` turns one back into references. The cost of linking is pinning: a linked deck runs whatever is installed, which is why behavioural change stays behind `Decklight.init` options and `present --check` says when the deck's recorded version and the served runtime differ by a major.
