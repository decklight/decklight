...
- Speaker notes: `<aside class="notes">`. Two markers inside them are timing, never words: `⟨CLICK⟩` cuts the notes into beats, one per build step (BUILDS), and `⟨PAUSE⟩` holds a let-it-sink-in beat where it stands (PRESENTING).
