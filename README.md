...
**Solo** (the picker's `Solo` row, or the palette's "Character solo"): hand the narrator the whole stage — slide content steps aside and the character centers, big. A talking-head slide without authoring one, and one keystroke back to the deck. It persists per deck like the rest of the character settings, and it can't strand you on a blank stage: the slides come back the moment the character leaves.
