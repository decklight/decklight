...
// Demo evidence for `decklight init --open` (#52) — a CLI feature, so the
// screenshot surface is the transcript. This script runs the real CLI twice
// and renders both transcripts into one terminal-styled HTML for shot.mjs:
//
...
//   2. with an empty PATH — no launcher anywhere — showing the graceful dim
//      skip line and the exit code staying 0.
//
...
//   node tools/shot.mjs .shots/init-open-demo/deck.html -o .shots/init-open-deck.png --drive shots/init-open.mjs --wait 3000
...
const headlessDir = path.join(demo, 'headless');
...
<p class="caption">headless box with no launcher anywhere on PATH: one dim line, exit 0 — the deck was created, which is the product</p>
`);
