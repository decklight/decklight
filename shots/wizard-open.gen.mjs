...
// Evidence for #233 — the engine wizard's player half is REACHABLE. The wizard
// only exists while an author server serves the deck, and tools/shot.mjs shoots
...
// /edit/wizard handing over a vetted schema) into a small REAL deck. Everything
// on screen — palette row, form, password field — is the real bundled engine
// rendering it; only the loopback server is stubbed, exactly as in
// test/engine.html.
//
...
<head>
  ...
  <script>
    ...
    window.fetch = async (url, init) => {
      ...
      if (u.includes('/edit/wizard') && init?.method !== 'POST') return json({ ok: true, schema: SCHEMA });
