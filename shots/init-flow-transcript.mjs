...
// Evidence for issue #92 (the #50 half) — init's git offer, dim status lines,
// accent epilogue with the clickable file:// link, and the dev handoff.
...
//
//   node shots/init-flow-transcript.mjs   → .shots/init-flow.png
...
if (piped.status !== 0) throw new Error(piped.stderr);
if (/\x1b/.test(piped.stdout)) {
  throw new Error('piped output contains escape codes — the TTY gate is broken');
}
...
const page = path.join(root, '.shots', 'init-flow-transcript.html');
fs.mkdirSync(path.dirname(page), { recursive: true });
fs.writeFileSync(page, html);
execFileSync('node', [path.join(root, 'tools', 'shot.mjs'), page,
  '-o', path.join(root, '.shots', 'init-flow.png'), '--size', '1280x1200', '--wait', '800'],
  { stdio: 'inherit' });
