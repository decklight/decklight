...
// Theme generator — SPEC THEMING/PRESENTING. Pure module (no DOM): generates a complete,
...
/**
 * A token the generated stylesheet will carry. `?gen=` is a shareable link
 * whose payload lands in a <style> element verbatim, so a value that can close
 * the declaration (`;`, `}`) or the comment the name sits in (`*\/`) is not a
 * token — it is a way to write any rule into somebody else's deck. Nothing the
 * generator itself emits contains any of these.
 */
const TOKEN_KEY = /^--[\w-]+$/;
export const safeTokenValue = (v) => !/[{};<>]|\/\*|\*\//.test(String(v));
export const safeTokens = (tokens) => Object.fromEntries(
  Object.entries(tokens ?? {}).filter(([k, v]) => TOKEN_KEY.test(k) && safeTokenValue(v)));

export function tokensToCss(name, tokens) {
  tokens = safeTokens(tokens);
  const label = String(name ?? '').replace(/\*\//g, '* /');
  let out = `/* ═══════════════════════════════════════════════════════════════
   Decklight theme · ${label}
   generated by Decklight (⌃T) — token contract: SPEC.md THEMING
